...
/*********************************************************************
 *
 *                  Tick Manager for Timekeeping Tick32_4.c
 *
 *********************************************************************
 * FileName:        Tick32_4.c
 ...
 * SH                   14 Jan. 2021 			Timer4 tested OK
 *
...
void TickInit(void)
{
...
#else
	// Use Timer 4 for 16-bit and 32-bit processors
	// 1:256 prescale
    PR4 = 0xffff; // set period register
    T4CONbits.TCKPS = 7; // set prescaler 
	...
	//	IEC0SET = _IEC0_T1IE_MASK;
		mT4SetIntPriority( 1);
        //INTSetPriority(_TIMER_4_VECTOR, INT_PRIORITY_LEVEL_1); // 1 <= p <= 7
        //INTSetSubPriority(_TIMER_4_VECTOR, 1); 
	#endif

	// Start timer
    INTEnableSystemMultiVectoredInt();
    //INTEnable(_TIMER_4_VECTOR, 1);  // EN = 1 for enable, 0 for disable
    mT4IntEnable(1);
#endif
}
...
static void GetTickCopy(void)
{
	...
	do
	{
		...
		// Get low 2 bytes
		((volatile WORD*)vTickReading)[0] = TMR4;
...
void __ISR( _TIMER_4_VECTOR, IPL1SOFT) T4InterruptHandler( void){
	// Increment internal high tick counter
	dwInternalTicks++;
